...
import nongsanantoan.admin.service.CustomerService;
import nongsanantoan.admin.validator.CustomerValidator;
		...
		try {
			...
			if (result.hasErrors()) {
				...
				SendMail.sendMail(customer.getEmail(), "Xin Chào, Thành Viên: " + customer.getFullName(),
						"Bạn bây giờ đã có thể đăng nhập vào cửa hàng Nông Sản An Toàn. Hãy tha hồ mua sắm nhé :)))"
